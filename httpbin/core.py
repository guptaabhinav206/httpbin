    ...
    if request.method == 'GET':
        ...
        if ctime is not None:
            try:
                ...
                bad_params.append('ctime')
        if limit is not None:
            try:
                limit = int(limit)
            except ValueError:
                bad_params.append('limit')

        if len(bad_params):
            r = make_response("",400)
            r.data = {"error": "url arguments {0} must be int".format(', '.join(bad_params))}
...
def return_files(dir_path, name=None, ctime=None, limit=None):
    """Returns a single file or multiple files from the directory as multipart/form-data"""
    limit = app.config['MAX_FILES_TO_RETURN'] if limit is None else limit

    if name is not None:
        return send_from_directory(dir_path, name)
    else:
        # get all the files from the directory
        file_paths = get_all_files_in_dir(dir_path=dir_path, ctime=ctime, limit=limit)

        if not file_paths:
            return make_response("", 204)

        data, headers = encode_multipart(file_paths)
